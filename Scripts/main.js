        ...
        function atualizarInterfaceAcesso() {
            ...
            if (isAdm()) { menuAdmin.classList.remove('hidden'); drawerAdmin.classList.remove('hidden'); }
            else { menuAdmin.classList.add('hidden'); drawerAdmin.classList.add('hidden'); if (paginaAtual === 'admin') { switchPage('dashboard'); return; } }
            ...
            if (paginaAtual) {
                ...
                const pageMap = { dashboard: ['dashboard-page','menu-dashboard','CONTROLE DE INFORMAÇÕES'],
        ...
        function atualizarPerfilInterface() {
            ...
            if (headerProfileRole) { headerProfileRole.textContent = tagsHtml || 'Membro'; headerProfileRole.className = 'profile-sub ' + (principal || ''); }
            if (drawerProfileRole) { drawerProfileRole.textContent = tagsHtml || 'Membro'; drawerProfileRole.className = 'profile-sub ' + (principal || ''); }
            ...
            const mobNick = document.getElementById('mob-menu-nick');
            const mobRole = document.getElementById('mob-menu-role');
            const mobAvatar = document.getElementById('mob-menu-avatar');
            if (mobNick) mobNick.textContent = USUARIO_ATUAL;
            if (mobRole) mobRole.textContent = tagsHtml || 'Membro';
            if (mobAvatar) mobAvatar.src = avatarHeadUrl(USUARIO_ATUAL);
        }
        ...
        function renderAdminMembrosAcesso() {
            ...
            if (!container) return;
            const acessos = DADOS.acessos_membros || {};
            const membros = Object.values(acessos).sort((a, b) => {
                return new Date(b.ultimo_acesso || 0) - new Date(a.ultimo_acesso || 0);
            });
            if (membros.length === 0) {
        ...
        async function init() {
            ...
            if (temAcesso()) {
                ...
                if (isAdm()) document.getElementById('mobitem-admin')?.classList.remove('hidden');
                ...
                function openDrawer() { overlay.style.display='block'; drawer.style.transform='translateX(0)'; document.body.style.overflow='hidden'; openBtn?.classList.add('hidden'); openBtnMobile?.classList.add('hidden'); }
                function closeDrawer() { drawer.style.transform='translateX(-100%)'; overlay.style.display='none'; document.body.style.overflow=''; openBtn?.classList.remove('hidden'); openBtnMobile?.classList.remove('hidden'); }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            window.switchPage = function(pageId) {
                ...
                document.getElementById('mob-topbar-title').textContent = titles[pageId] || '';
            };

            const origAdmin = window.openAdminPanel;
            window.openAdminPanel = function() {
                origAdmin();
                ['dashboard','search','info','admin'].forEach(id =>
                    document.getElementById('mobitem-' + id)?.classList.remove('active'));
                document.getElementById('mobitem-admin')?.classList.add('active');
                document.getElementById('mob-topbar-title').textContent = 'PAINEL DE CONTROLE';
            };
